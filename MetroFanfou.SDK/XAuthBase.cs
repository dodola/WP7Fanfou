...
namespace MetroFanfou.SDK
{
    public class XAuthBase
    {
        ...
        private string ComputeHash(HashAlgorithm hashAlgorithm, string data)
        {
            ...
            byte[] dataBuffer = Encoding.GetEncoding("utf-8").GetBytes(data);
        ...
        /// <param name="signatureType">The signature type. To use the default values use <see cref="OAuthBase.SignatureTypes">OAuthBase.SignatureTypes</see>.</param>
        /// <returns>The signature base</returns>
        public string GenerateXSignatureBase(Uri url, string consumerKey, string token, string tokenSecret,
                                             string httpMethod, string timeStamp, string nonce, string signatureType,
                                             string username, string password, out string normalizedUrl,
        ...
        /// <param name="httpMethod">The http method used. Must be a valid HTTP method verb (POST,GET,PUT, etc)</param>
        /// <returns>A base64 string of the hash value</returns>
        public string GenerateXSignature(Uri url, string consumerKey, string consumerSecret, string token,
                                         string tokenSecret, string httpMethod, string timeStamp, string nonce,
                                         string username, string password, out string normalizedUrl,
                                         out string normalizedRequestParameters)
        {
            return GenerateXSignature(url, consumerKey, consumerSecret, token, tokenSecret, httpMethod, timeStamp, nonce,
                                      SignatureTypes.HMACSHA1, username, password, out normalizedUrl,
                                        ...
                                        out string normalizedRequestParameters)
        {
            ...
            switch (signatureType)
            {
                ...
                case SignatureTypes.HMACSHA1:
                    ...
                    hmacsha1.Key =
                        Encoding.GetEncoding("ASCII").GetBytes(string.Format("{0}&{1}", UrlEncode(consumerSecret),
        ...
        public string GenerateXSignature(Uri url, string consumerKey, string consumerSecret, string token,
                                         string tokenSecret, string httpMethod, string timeStamp, string nonce,
                                         SignatureTypes signatureType, string username, string password,
                                         out string normalizedUrl, out string normalizedRequestParameters)
        {
            normalizedUrl = null;
            normalizedRequestParameters = null;
            ...
            switch (signatureType)
            {
                case SignatureTypes.PLAINTEXT:
                    return HttpUtility.UrlEncode(string.Format("{0}&{1}", consumerSecret, tokenSecret));
                case SignatureTypes.HMACSHA1:
                    string signatureBase = GenerateXSignatureBase(url, consumerKey, token, tokenSecret, httpMethod,
                                                                  timeStamp, nonce, HMACSHA1SignatureType, username,
                                                                  password, out normalizedUrl,
                                                                  out normalizedRequestParameters);

                    var hmacsha1 = new HMACSHA1();
                    hmacsha1.Key =
                        Encoding.GetEncoding("utf-8").GetBytes(string.Format("{0}&{1}", UrlEncode(consumerSecret),
                                                                             string.IsNullOrEmpty(tokenSecret)
                                                                                 ? ""
                                                                                 : UrlEncode(tokenSecret)));
